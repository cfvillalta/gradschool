...
#I am just printing out the median differences. Where the meidan is just between bioreps.    
    ...
    $stat_before->add_data(($row[1]/$totals{1}),($row[2]/$totals{2}),($row[4]/$totals{4}));
    my $median_before = $stat_before->median();
    
    my $stat_after = Statistics::Descriptive::Full->new();
    $stat_after->add_data(($row[3]/$totals{3}),($row[5]/$totals{5}),($row[6]/$totals{6}));

    my $median_after = $stat_after->median();

    my $b1 = log($row[1]/$totals{1})-log($median_before);
    my $b2 = log($row[2]/$totals{2})-log($median_before);
    my $b3 = log($row[4]/$totals{4})-log($median_before);
    
    my $a1 = log($row[3]/$totals{3})-log($median_after);
...
#I am going to print out the median differences where the median is from all the lanes of data. The biorep normalized read count is subtracted from that.
 ...
 #   print "$b2\t$b_2\t$median_before\n";
    ...
    $stat_all->add_data(($row[1]/$totals{1}),($row[2]/$totals{2}),($row[4]/$totals{4}),($row[3]/$totals{3}),($row[2]/$totals{2}),($row[6]/$totals{6}));
    my $median_all = $stat_all->median();
    my $b1_2 = log($row[1]/$totals{1})-log($median_all);
    my $b2_2 = log($row[2]/$totals{2})-log($median_all);
    my $b3_2 = log($row[4]/$totals{4})-log($median_all);
    
    my $a1_2 = log($row[3]/$totals{3})-log($median_all);
    my $a2_2 = log($row[5]/$totals{5})-log($median_all);
    my $a3_2 = log($row[6]/$totals{6})-log($median_all);
