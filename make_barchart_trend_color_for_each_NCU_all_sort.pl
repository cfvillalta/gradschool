...
foreach my $x(reverse(sort{$con2{$a}cmp$con2{$b}} keys %con2))
{
    push(@col_data2,$col{$con2{$x}});
}
foreach my $x(reverse(sort{$con3{$a}cmp$con3{$b}} keys %con3))
{
    push(@col_data3,$col{$con3{$x}});
}
foreach my $x(reverse(sort{$con4{$a}cmp$con4{$b}} keys %con4))
{
